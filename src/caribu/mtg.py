"""
Methods for interacting with mtg and/or lpy
...
def shape_to_can(shape,tesselator,optid = 1,opak = 0,plant_id = 1,elt_id = 1):
    ...
    return [_line(ind, pts, _label)+'\n' for ind in indices]

def lpy_to_canestra(scene,opt = 1,opak = 0,plant_index=1):
    """
    Build canestra file representation of scene and a list of id triangles belongs to.
    OptId and Opak are the name of mtg properties to use.
	...
	_label = label.Label()
	_label.plant_id = plant_index
	_label.optical_id = opt
	_label.leaf_id = opak
	_label.elt_id = 1
	l = (_line(ind, pts, _label)+'\n' for ind in indices)
	output.extend(l)
	id.extend([shape.id]*len(indices))
