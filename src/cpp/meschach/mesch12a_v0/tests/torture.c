...
char	*argv[];
{
   ...
   char	*cp;
    ...
    A = m_resize(A,53,12) ;			/* WAS 11,12 ); HA 09 2001 */
	...
	else
	{
	    M_FREE(B);
	    B = m_load(fp,&cp) ;
	    if ( strcmp(name,cp) || (test_err = m_norm1(m_sub(A,B,B))) >= MACHEPS*A->m )
	      {
		errmesg("m_load()/m_save()");
                printf ("m_norm1(m_sub(A,B,B)) * MACHEPS * A->m  = \
 %lf * MACHEPS * %d = %lf\n",
			test_err,
			A->m,
			test_err * (double)A->m * MACHEPS);
	        printf ("name = %s ; cp = %s\n", name, cp ) ;
	      }
	}
    }
