...
extern  SPMAT	*spLUfactor(SPMAT *A, PERM *px, double alpha)
{
...
/* spLUsolve -- solve A.x = b using factored matrix A from spLUfactor()
	-- returns x
	-- may not be in-situ */
extern  VEC	*spLUsolve(SPMAT *A, PERM *pivot, VEC *b, VEC *x)
{
	int	i, idx, len, lim;
	Real	sum, *x_ve;
	SPROW	*r;
...
/* spLUTsolve -- solve A.x = b using factored matrix A from spLUfactor()
	-- returns x
	-- may not be in-situ */
extern  VEC	*spLUTsolve(SPMAT *A, PERM *pivot, VEC *b, VEC *x)
{
...
/* spILUfactor -- sparse modified incomplete LU factorisation with
	...
	-- in situ factorisation */
extern  SPMAT	*spILUfactor(SPMAT *A, double alpha)
{
