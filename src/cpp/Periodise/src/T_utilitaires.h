...
Type & Tabdyn<Type,D>::operator()(int first){
  if(D==1)
    ...
    fprintf(stderr,"Tabdyn<Type,D>::operator()(int first) incompatbile avec D=%d\n",D);
    exit(-1);
...
Type & Tabdyn<Type,D>::operator()(int first, int second){
  if(D==2)
    ...
    fprintf(stderr,"Tabdyn<Type,D>::operator()(int first,int second) incompatbile avec D=%d\n",D);
    exit(-1);
    return(tab[0]);// pour eviter les wranings verbeux de SGI !!!
  } 
}//operator ()
...
Type & Tabdyn<Type,D>::operator()(int first, int second, int third,...){
  /*  if(tab==NULL) {
    cerr<< "Tabdyn [operator()]: Tableau dynamique pas alloue : acces impossible!\n";
    exit(-1);
  }
  */
  va_list ptarg;
